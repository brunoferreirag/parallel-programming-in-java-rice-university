...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
       return Arrays.stream(studentArray).parallel().filter(p->p.checkIsCurrent()).mapToDouble(p->p.getAge()).average().getAsDouble();
    }
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        return Arrays.stream(studentArray).parallel().filter(p->!p.checkIsCurrent())
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        return Integer.valueOf(String.valueOf(Arrays.stream(studentArray).parallel().filter(p->!p.checkIsCurrent() && (p.getAge() > 20 && p.getGrade() < 65)).count()));
    }
}
